...
or open `iv.ipynb` in Jupyter and run all cells. Without `ETROC_FIGURES` the figures go to
`figures/iv/` in the notebook's folder; check them from `TestBeam/` with
...
point the notebook at your own copy. The notebook's first code cell checks every input and stops
with a list of whatever is missing or unreadable.
...
`iv.ipynb` and `iv.py` are the same notebook, paired by jupytext (`py:percent`). Edit either one,
...
2. Start the campaign's notebook from a copy of `notebooks/iv.py` and set `CAMPAIGN = "<name>"` in
   its setup cell. `iv.ipynb` itself is written for IRRAD 2026: its per-figure choices name that
   campaign's scans, chips and runs. An exported `ETROC_CAMPAIGN` must match `CAMPAIGN` (the
...
the July HV-monitor extracts). A campaign recorded in other formats needs loaders of its own; the
style, the checks and the saving code do not depend on the format.
