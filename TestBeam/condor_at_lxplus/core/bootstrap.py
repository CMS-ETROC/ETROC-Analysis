...
def fit_gmm_and_get_fwhm(data: np.ndarray):
    ...
    best_fwhm, best_ks, best_p = 0.0, 1.0, 0.0
...
def run_sample_analysis(sample_df: pd.DataFrame, roles: list[str], threshold: float, is_boot: bool):
    ...
    It used to be accepted if the KS DISTANCE was at most 0.03, but the KS
    distance of a perfectly good fit scales like ~0.83/sqrt(N): with 300 events
    the median good fit has D ~ 0.048 and ~80% of resamples were rejected for
    nothing more than statistics (the accepted ones being the lucky
    fluctuations, i.e. a biased selection), while with 20000 events D ~ 0.009
    and the same rule accepted everything. The p-value is N-independent by
    construction; the default (0.001) is lenient on purpose -- this gate is
    meant to throw out broken fits (spikes, degenerate mixtures, D >~ 0.1 at
    any N), not to test how perfect a 3-component mixture is at high
    statistics (with 20k events even a fine fit sits at p ~ 0.05).

    Besides the per-board resolutions, the returned dict carries the pair
    widths themselves (FWHM/2.355 of each pair) under keys 'pair_<a>-<b>', so
    a downstream consistency check can use the pipeline's own numbers.
    """
    ...
    for pair, data in diffs.items():
        ...
        if ks_p < threshold:
            logger.info(f"[{label}] Rejecting pair {pair}: KS p-value {ks_p:.2e} < {threshold:.1e} (D {ks:.4f})")
            return None, False
...
def main():
    ...
    parser.add_argument('--ks_pmin', type=float, default=1e-3,
                        help='Minimum KS p-value for a pair\'s mixture fit to be accepted (default 1e-3). '
                             'N-independent replacement for the old "KS distance <= 0.03" rule, which rejected '
                             'most good fits below ~1000 events and none above; see run_sample_analysis(). '
                             'Relaxed by halving after each failed single-shot attempt / every 100 fruitless '
                             'bootstrap attempts, down to --ks_pmin_floor.')
    parser.add_argument('--ks_pmin_floor', type=float, default=1e-6,
                        help='Floor of the relaxed --ks_pmin (default 1e-6); below it the phase gives up.')
    ...
    for phase_idx, (target, is_boot) in enumerate(phases):
        ...
        current_threshold = args.ks_pmin
        logger.info(f"Starting Phase: {'Bootstrap' if is_boot else 'Single-Shot'} (KS p-value >= {current_threshold:.1e})")
        ...
        while n_success < target and attempts < max_attempts:
            ...
            if not is_boot:
                # Relax the p-value threshold every failed attempt for the fixed 100% dataset
                if n_success == 0 and current_threshold > args.ks_pmin_floor:
                    current_threshold *= 0.5
            else:
                # Standard relaxation for Bootstrap resamples: only while nothing succeeded yet
            ...
            # For Single-Shot, if we hit the floor and still fail, exit the while loop
            if not is_boot and current_threshold <= args.ks_pmin_floor and n_success == 0:
                break
