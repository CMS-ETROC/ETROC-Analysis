...
Where things are read from and written to. The input `-p` is resolved under your EOS base when relative, but an *absolute* path is used as-is - so you can read a colleague's decoded feathers directly (e.g. `-p /eos/user/j/jongho/.../Run_1_feather`) without copying them. The outputs, on the other hand, are written relative to the current working directory, not to EOS: `--cal-label` is a filename *prefix* (`<CAL_LABEL>_cal_table.csv`), `--track-label` is a *directory*, and the alignment yaml goes to `<TRACK_LABEL parent>/alignment/`. Run this step from the directory where you want the outputs to live (e.g. a work directory on your EOS), or pass absolute output paths.

Spatial (radius) cut. `--max_diff_pixel` is a coincidence-consistency window, not a straight-line fit: for every board in the combo, its transformed pixel centre must lie within `max_diff_pixel x 1.3 mm` of the *anchor* board's pixel centre. The anchor is the trigger board when it is in the combo; for a combo without the trigger board (e.g. `ref1-dut2-extra3`) the anchor falls back to ref, then dut, then extra (before this fallback such combos received no cut at all). The window tolerates inclined tracks and board offsets up to its size; the yaml `transformation` (translation and, if given by hand, rotation) is what centres it.
...
| `--max_diff_pixel` | `1` | Radius of the spatial coincidence window, in pixels (x 1.3 mm), around the anchor board's pixel (trigger board if in the combo, else ref/dut/extra). |
...
| `--find_alignment` | off | Estimate each non-trigger board's translation relative to the trigger board, for every combo that contains it. The estimate is the count-weighted mode of the integer pixel shift refined by the centroid over mode +/- 1 pixel (a sub-pixel value; it absorbs mechanical misalignment *and* the mean projected track offset, which is what centres the radius window). Each board's estimate is applied in-memory EXACTLY ONCE, from the first trigger-containing combo that contains it, so on a telescope of N >= 4 boards every non-trigger board is aligned (an earlier version applied one combo's estimate only, which left one board on its `--config` value - see `docs/fixes_2026-09_alignment.md`); trigger-containing combos are processed first so no combo is ever cut on uncorrected geometry. Every combo's estimate is written, keyed by combo, to `<TRACK_LABEL parent>/alignment/{TRACK_LABEL basename}_alignment.yaml`, plus an `applied:` block naming which combo supplied each board's value. Nothing is saved back into `--config` - merge in manually, and record the geometry it is valid for. Requires a board with role `trig` (the run errors out otherwise instead of silently estimating nothing). |
| `--alignment_core_warn` | 0.10 | With `--find_alignment`: warn when less than this share of a combo's candidate weight sits within +/-1 pixel of the modal shift, i.e. when the applied translation may be the combinatorial-background mode rather than the beam spot (a board outside the trigger's acceptance does this). Warn-only, the value is still applied; 0 disables. A conservative floor, not a calibrated discriminator - set it per campaign. |
...
- Landing peak and spread (`track_landing.png`): peak = the most common whole-pixel shift, refined by the count-weighted average of the shifts within +-1 pixel of it (the same estimator `path_finder.py --find_alignment` uses); spread = the count-weighted standard deviation of those shifts.
