...
| `utils/quote_resolution.py` | step 13 | Quotable per-board numbers with one fixed recipe: cleaning, per-pixel table, robust mean over pixels, partner and definition systematics, chip halves, illumination split (average and central-hit maps), the same value at two event thresholds, and - given a four-board width table from `utils/fourboard_pairing.py` - the pairing-covariance fit alongside. | [13b](#13b-quoting-the-resolutions---one-recipe-for-any-beam-campaign-and-number-of-boards) |
...
### 6. Path finding
```bash
python core/path_finder.py -p <PATH> --cal-label <CAL_LABEL> --track-label <TRACK_LABEL> -c <CONFIG> -r <RUNNAME> -s <SAMPLING> [--max_diff_pixel <N>] [--mask_config <MASK_YAML>] [--cal_table_only] [--find_alignment [--apply_alignment] [--alignment_core_warn <F>]] [--seed <N>] [--max_files <N>]
```
...
Where things are read from and written to. The input `-p` is resolved under your EOS base when relative, but an *absolute* path is used as-is - so you can read a colleague's decoded feathers directly (e.g. `-p /eos/user/j/jongho/.../Run_1_feather`) without copying them. The outputs, on the other hand, are written relative to the current working directory, not to EOS: `--cal-label` is a filename *prefix* (`<CAL_LABEL>_cal_table.csv`), `--track-label` is a *directory*, and the alignment yaml goes to `alignment/{TRACK_LABEL basename}_alignment.yaml` under the current working directory. Run this step from the directory where you want the outputs to live (e.g. a work directory on your EOS), or pass absolute output paths.

Spatial (radius) cut. `--max_diff_pixel` is a coincidence-consistency window, not a straight-line fit: for every board in the combo, its transformed pixel centre must lie within `max_diff_pixel x 1.3 mm` of the *anchor* board's pixel centre. The anchor is the trigger board when it is in the combo; for a combo without the trigger board (e.g. `ref1-dut2-extra3`) the anchor falls back to that combo's own median board id - the same anchor the `global_relative` alignment estimate uses (before this fallback such combos received no cut at all: `x_{trig}` was not among their columns, no condition was built, and every combinatorial pattern passed). The window tolerates inclined tracks and board offsets up to its size; the yaml `transformation` (translation and, if given by hand, rotation) is what centres it.

Alignment: two modes. `--find_alignment` on its own is **diagnostic** and is the default behaviour: it estimates every board's translation, writes it to `alignment/{TRACK_LABEL basename}_alignment.yaml`, and changes nothing else - the track output of a run with `--find_alignment` is identical to one without it. The yaml holds two independent estimates for comparison: `legacy_per_combo` (each board against the trigger board, per combo that contains the trigger) and `global_relative` (each board against its combo's own median board, every combo's measurements combined into one least-squares fit pinned at the trigger board). Both come from a 30-bin histogram of the shift distribution, so both are quantised to about half a pixel; read them as a check that the geometry in `--config` is roughly right, and merge a value in by hand if it is not.

Adding `--apply_alignment` (opt-in; it requires `--find_alignment`) turns the estimate into a **correction**: each non-trigger board's translation is measured with a sub-pixel estimator (count-weighted mode of the integer pixel shift, refined by the centroid over mode +/- 1 pixel) and fed forward into the in-memory run config, so every later combo's coincidence window is centred where the tracks actually land. Each board is aligned exactly once, from the first trigger-containing combo that contains it, and trigger-containing combos are processed first so no combo is ever cut on uncorrected geometry. The yaml then also carries an `applied:` block giving, per board, the translation its tracks were really cut with and the combo it came from; `legacy_per_combo` and `global_relative` still report against the `--config` geometry and are unchanged by the flag. **Track output does change** - this is a different reconstruction, not a diagnostic - and nothing is written back to `--config`.

Use `--apply_alignment` when the board-config yaml has no translations (or wrong ones) for the run. That was the DESY August 2026 case: every yaml translation was zero, board 3 sat about 13 mm from where the window was centred, and the three combos containing it produced only combinatorial background - one board of a 4-board telescope silently lost, on every run of the campaign (`docs/fixes_2026-09_alignment.md`). Two caveats when you do use it. First, `--alignment_core_warn` did **not** fire on a genuine background latch (DESY May 2026 run 5, board 0); the warning to trust is the cross-combo disagreement message, `board N already aligned from (combo) as ...; this combo re-measures ... (max residual X mm)` - a large residual there means the applied translation is not to be believed, and that run's geometry should be supplied instead. Second, the durable fix is not this flag: measure the telescope geometry once per holder configuration and put the translations in the board-config yaml, then `--find_alignment` reads back a residual of about zero. For August 2026 that is already done - `board_configs_yaml/DESY_TB_2026Aug_corrected.yaml` carries the measured geometry and is the preferred route; pass it with `-c` and no `--apply_alignment` is needed.
...
| `--find_alignment` | off | **Diagnostic only.** Estimate every board's translation two ways and write both to `alignment/{TRACK_LABEL basename}_alignment.yaml`: `legacy_per_combo` (each board against the trigger board, for every combo containing the trigger) and `global_relative` (each board against its combo's own median board, all combos combined into one least-squares fit pinned at the trigger). Both use the 30-bin histogram estimator and are quantised to roughly half a pixel. Never mutates `--config` or the in-memory geometry, so the saved track output is identical with and without it. Merge a value in by hand if you want it applied, and record the geometry it is valid for. |
| `--apply_alignment` | off | **Opt-in; requires `--find_alignment`, and it changes the track output.** Feed the alignment estimate forward into the in-memory run config so later combos are cut on corrected geometry. Uses a sub-pixel estimator (count-weighted mode of the integer pixel shift, refined by the centroid over mode +/- 1 pixel; it absorbs mechanical misalignment *and* the mean projected track offset, which is what centres the radius window), not the diagnostic histogram one. Each board is aligned EXACTLY ONCE, from the first trigger-containing combo that contains it, so on a telescope of N >= 4 boards every non-trigger board gets a translation; trigger-containing combos are processed first so no combo is cut on uncorrected geometry. Adds an `applied:` block to the alignment yaml naming each board's value and the combo it came from, and logs a cross-combo residual for every board a later combo re-measures. Requires a board with role `trig` (errors out otherwise instead of silently applying nothing). Nothing is written back to `--config`. |
| `--alignment_core_warn` | 0.10 | With `--apply_alignment` (ignored otherwise): warn when less than this share of a combo's candidate weight sits within +/-1 pixel of the modal shift, i.e. when the applied translation may be the combinatorial-background mode rather than the beam spot (a board outside the trigger's acceptance does this). Warn-only, the value is still applied; 0 disables. A conservative floor, not a calibrated discriminator - and it is known to miss real background latches, so treat the cross-combo residual message as the primary warning. Set it per campaign. |
...
```
Worked example (H1, run 1, outputs under the user's EOS `CERN_IRRAD_Mar2026/analysis/`; run from `condor_at_lxplus/` with `source envs/load_python39.sh` -- step 6 writes the alignment yaml relative to the working directory it was run from, not under `$E`):
```bash
...
- Landing peak and spread (`track_landing.png`): peak = the most common whole-pixel shift, refined by the count-weighted average of the shifts within +-1 pixel of it (the same estimator `path_finder.py --apply_alignment` uses); spread = the count-weighted standard deviation of those shifts.
...
Each saved step-10 row also carries `src_row`, the row label it had in this step's input (the step-9 track table). The saved index itself is a plain `RangeIndex`, so code that assumed the index survived the cuts joined 39-45 % of its rows to the wrong events; use `core/apply_tdc_cuts.join_step9_rows(step10_df, step9_df)`, which uses `src_row` and raises rather than guessing when it is absent (tables written before it existed cannot be joined at all).
...
```bash
python submit/submit_bootstrap.py -d <DIRNAME> -o <OUTPUTDIR> -n <NUM_BOOTSTRAP_OUTPUT> --minimum_nevt <MINIMUM_NEVT> --iteration_limit <ITERATION_LIMIT> --condor_tag <CONDOR_TAG> [--reproducible] [--ks_pmin <P>] [--ks_pmin_floor <P>] [--ks_dmax <D>] [--gmm_tol <TOL>] [--gmm_max_iter <N>] [--gmm_reg_covar <F>] [--neighbor_cut <COL> ...] [--neighbor_logic <OR|AND>] [--dryrun]
```
| Flag | Default | Description |
|---|---|---|
...
| `--minimum_nevt` | `300` | Minimum event count required to run bootstrap (submitter default moved 1000 -> 300; `bootstrap.py`'s own default moved 100 -> 300 alongside it, so the two are no longer mismatched -- pass `--minimum_nevt 1000` to restore the submitter's old cut; see [docs/fixes_2026-09_quote_and_event_floor.md](docs/fixes_2026-09_quote_and_event_floor.md)). |
...
| `--ks_pmin` | `1e-3` | A pair's mixture fit is accepted if its KS p-value is at least this or its KS distance is at most `--ks_dmax` (relaxed by halving after failed attempts down to `--ks_pmin_floor`, `1e-6`, tried once; the bootstrap phase starts at the threshold the single-shot was accepted at). The former rule was the distance alone (D ≤ 0.03), a statistical rather than a quality gate: the distance of a perfectly good fit scales like 0.83/√N, so it rejected most good fits below ~800 events (accepting only fluctuations, a biased selection) and tested nothing above ~3000. For a correct, converged model the p-value is N-independent (good fits sit at p ~ 0.9 at any N); in distance terms p = 1e-3 is D ≈ 0.19 / 0.11 / 0.06 / 0.014 at N = 100 / 300 / 1000 / 20000. |
| `--ks_dmax` | `0.03` | A fit within this KS distance of the data is always accepted, so the accepted set is a superset of the old rule's at every N (a very large sample cannot reject a fit for a small model imperfection). |
| `--gmm_reg_covar` | `4.0` | Variance floor in ps² on every mixture component (a 2 ps σ floor). With the converged settings below, EM at low N has time to run into the Gaussian-mixture likelihood singularity - a component collapses onto one quantised point, σ → 0 - which happened on ~1.9 % of fits at N = 300 and on 2 of 9 real low-N pairs; the spike then owns the PDF maximum, no half-maximum width can be read off and the resample is silently discarded, so it costs bootstrap efficiency and biases the accepted set. The floor is a fifth of the ~10 ps TDC quantisation floor of a pairwise TOA difference (LSB 3.125 ns / 128 = 24.4 ps, uniform sd 7.05 ps, √2× for a difference), so it cannot bias anything physically resolvable, and it widens a 45 ps mixture by 0.04 ps (0.1 %, an order of magnitude below the 1 % definition systematic). |
| `--gmm_tol`, `--gmm_max_iter` | `1e-6`, `2000` | EM convergence of the Gaussian-mixture fit. sklearn's defaults (`1e-3`, `100`) leave the fit unconverged on these distributions - EM stops after ~10 iterations with `converged_ = True`, the peak is too high and narrow, and FWHM/2.355 comes out ~12 % low on tracks with thousands of events (measured 43.4 → 49.2 ps on a 21k-event pair, σ_dut 31.6 → 35.6 ps), with KS p-values ~0.05-0.1 that become ~0.9-0.99 once converged. The residual change between `1e-6` and `1e-7` (~0.7 %) is the intrinsic softness of an FWHM-of-a-mixture width and should be quoted as a systematic. Fits are ~3× slower. |
...
**The time-walk correction is a single joint least-squares solve** (`core/twc_solver.py`, shipped to the workers alongside `bootstrap.py`). For the three boards of a track it minimises Σ over the three board pairs of ‖(TOA_a + P_a(TOT_a)) − (TOA_b + P_b(TOT_b))‖² over all nine quadratic coefficients at once. It used to be an alternating two-pass loop (`for _ in range(2)`), i.e. a fixed-point iteration of that same problem stopped after two passes: the leftover own-TOT slope after k passes is s₁·ρ^(k−1) with ρ the inter-board TOT correlation, so at ρ ≈ 0.35 (heavily irradiated boards, correlated TOT) two passes left 26-30 ps/ns un-corrected and pair widths 7-16 ps too wide, while at ρ ≈ 0.1 the same defect is worth < 0.25 ps. The joint solve is exact, has no iteration count to tune, and agrees with the loop run to convergence to 2×10⁻¹¹ ps on the pair differences (up to one global constant, which enters no width) and to 0.00 ps on the pair widths. The polynomial stays quadratic in TOT: a cubic term is detectable in the mean shape but, cross-fitted, makes the widths marginally worse. The same function is used by `BeamTestHelpers/twc.py` and by both diagnostics tools, so the correction cannot drift between them.

#### 12b. (Optional) Bootstrap diagnostics: anatomy of a trial, statistical health, partner consistency
Once step 12 has run, `utils/bootstrap_diagnostics.py` shows what the resolution extraction actually did. It imports and re-uses `bootstrap.py`'s and `fit_bootstrap_results.py`'s own functions and constants (gate thresholds, EM convergence), so the fits it draws are the pipeline's (with one caveat: the Gaussian-mixture fit is unseeded in the pipeline, so a replay reproduces the procedure, not bit-for-bit numbers: typically ~0.1 ps apart for tracks with thousands of events, many ps where the mixture is unstable (a few hundred events) - both numbers are printed on the figure, together with the accept/reject verdict step 12 would give the replayed attempt).
```bash
...
Each `track_*_boot.parquet` holds one single-shot row (`is_bootstrap == False`) and up to `-n` bootstrap rows: the resolution per role (columns named by role), the three pair widths `pair_<a>-<b>` (FWHM/2.355 of the kept mixture) that the 3-board solve used, and `ksp_min`, the smallest KS p-value over the three pair fits of that sample. A phase that failed every attempt writes a single row of `-1` placeholders (all resolution columns). Every row also carries the track's **trial bookkeeping** as `trials_*` columns - `trials_boot_attempts`, `trials_boot_accepted`, `trials_boot_target`, `trials_boot_ks_pmin_final`, `trials_boot_accept_rate` and the same four for the single shot (`trials_ss_*`) - constant down the file. Until they existed only the number of accepted resamples survived (step 13 recovers it as `n_boot` from the row count), so the acceptance rate the job log warns about below 0.7 - the regime where the accepted resamples are a biased subset - could not be checked, filtered on or reported for any track after the fact. They are additive: anything iterating over the resolution columns must skip the `trials_` prefix (step 13's `TRIAL_PREFIX` does, including in the all-negative test that detects a failed bootstrap). The job log still carries the `[Summary]` line per phase.
...
Besides `res_*`, `err_*`, `single_shot_res_*` and the audit metrics, the table carries `single_shot_failed_<col>` (1 when step 12 wrote a `-1` placeholder for that track's single-shot, i.e. every full-sample attempt failed by KS or by an imaginary 3-board solve; the `res_/err_` of such a track come from whatever resamples were accepted and can be unphysical, e.g. an imaginary solve barely turned real - exclude or mark them downstream), `n_boot` (accepted bootstrap rows; 0 when the bootstrap phase failed), `boot_failed` (1 when the bootstrap phase itself never succeeded) and step 12's `trials_*` bookkeeping carried through verbatim (attempts, acceptances and final KS threshold per phase, plus `trials_boot_accept_rate`) - never fitted, never counted as a role. Note that a near-degenerate 3-board solve can also come out barely real (e.g. σ of a few ps next to pair widths of 60-100 ps) without any flag: check `res_pair_*` against `res_<role>` downstream. Pair widths appear as `res_pair_<a>-<b>` etc.

#### 13b. Quoting the resolutions - one recipe for any beam, campaign and number of boards
`utils/quote_resolution.py` turns the step-13 table(s) into quotable numbers with a fixed recipe, so a three-plane telescope (one table) and a four-board telescope (one table per leave-one-out combo) are treated identically. It produces two per-pixel maps and two board-level numbers per board: the average map (canonical - every track using the pixel, 1/err²-weighted, i.e. the operating resolution under this illumination) and the central-hit map (only tracks whose partner pixels sit at the EVENT-WEIGHTED modal offset - the offset with the largest summed weight, nevt when available else 1/err², not a plain per-track vote, which can flip to a side offset as selection depth grows even though the nominal offset keeps the most events - one sub-region of the pixel, the more geometry-independent number for chip-to-chip comparisons); both maps are also written as CSV (`<label>_map_<board>_{average,central}.csv`):
```bash
...
```
The markdown table now has 16 columns (was 12: two nevt-threshold columns and two vote-margin diagnostics columns were added, see below); the JSON gained the keys `boards.<role>.nevt_thresholds`, `pairing_covariance`, and the vote-diagnostics fields `boards.<role>.central_diagnostics_summary` / `.pixel_mode_match_summary` -- any downstream parser of `*_resolution_quote.md`/`.json` that assumes a fixed column count or key set must be updated.

Recipe: (1) drop tracks with `single_shot_failed`/`boot_failed`, fewer than `--min-boot` resamples, or a near-degenerate solve (solved σ below `--margin-lo` = 0.35 or above `--margin-hi` = 0.95 of the smallest pair width it came from); (2) per pixel, the 1/err²-weighted mean of `res_` over the tracks using it (the standard resolution table) plus the spread of `res_` over those tracks; (3) per board and table, the robust Gaussian mean over pixels (the same `perform_robust_unbinned_fit` step 13 uses) = the value, its width = pixel-to-pixel spread (quoted as spread), stat error = width/√N_pix; (4) systematics with the same names everywhere: partner = per pixel, half the range of that pixel's per-table values, median over the pixels present in ≥ 2 tables (so different pixel coverage per table - an angled beam fires different rows in different combos - cannot fake a partner shift), or, with one table or no pixel in two tables, the median excess of the track-to-track spread over the bootstrap error per pixel (how much the answer depends on which partner pixels were used - exists with three boards); definition = `--def-syst` (1 %, the convergence softness of the FWHM-of-a-mixture width; the core-vs-RMS convention is stated, not folded in); chip halves = error-weighted left − right, reported; illumination = the same pixel measured by tracks whose partners sit at the event-weighted modal offset (one sub-region of the pixel, set by the fractional inter-plane alignment) versus off-nominal partners (the complementary side sub-region, a few hundred µm at 1.3 mm pitch, not a charge-sharing edge): the quoted value is the event-weighted average of both (the operating resolution under this alignment/beam), and the central-only value, the off-nominal − central difference and the off-nominal event share are reported (event weights from the step-11 `nevt_*.csv` if present next to the tables); (5) the quoted number = the robust Gaussian mean over the COMBINED per-pixel map (all tables' cleaned tracks pooled per pixel; a pixel present in one table only just gets that table's tracks) ± stat ± partner ± definition; coverage (pixels per table, in the union, in ≥ 2 tables) is reported. With ≥ 4 boards the least-squares solve of all pair widths is printed as a consistency check (never as the number). Worked example: `python utils/quote_resolution.py -i "/eos/user/m/musafdar/CERN_IRRAD_Mar2026/analysis/final_h1_run1/resolution_table_*.csv" -o .../final_h1_run1/quote --label h1_run1`.

**Central-hit modal-offset diagnostics and the `--alignment` cross-check.** (The two defects this fixed and their regression numbers: [docs/fixes_2026-09_quote_and_event_floor.md](docs/fixes_2026-09_quote_and_event_floor.md).) Two extra markdown/JSON columns report how well-resolved the central-hit definition is: `modal/runner-up (min)` is the smallest weight-share ratio, over every (table, partner board) pair, of the winning offset over the next most common one (below 1.5× the split is thin - typically a near-half-pixel misalignment - and a WARNING is printed); `pixel-mode match (min)` is the smallest fraction of a board's pixels whose OWN most-weighted partner offset agrees with the table-level mode (a rigid-translation check that a real rotation would fail even with a clear table-level vote). Full per-table, per-partner detail is in the JSON under `boards.<role>.illumination_per_table.<combo>.{central_diag,pixel_mode_match}`. Optionally, `--alignment <track_label>_alignment.yaml` (step 6's alignment output) compares the translation-implied pixel offset against the weighted modal offset and warns on mismatch. It needs the translations the tracks were actually cut with, so it reads the `applied:` block written by `--apply_alignment` (or an older flat per-combo yaml); a diagnostic-only `--find_alignment` yaml has no such block and is reported as unusable rather than compared against. The check is conservative by construction: role↔board-id is inferred from the yaml's own combo-label keys, a board with no recorded translation that is not the trigger board is flagged rather than silently assumed zero, and **the column axis is not compared at all unless `--config <board-config-yaml>` is also given**, because a y-axis telescope tilt rescales it (`x_global = (col − 7.5)·pitch·cos(rotation.y) + translation.x`, so a "small rotation" assumption is wrong on most DESY-May runs - `rotation.y` runs −10° to −60° in `board_configs_yaml/DESY_TB_2026May_CE.yaml`) while the row axis is rotation-invariant and always compared. With `--config`, the column prediction uses the mean of the two boards' own `rotation.y` (exact when they share one telescope tilt, as observed on every DESY-May run checked; flagged as approximate otherwise).

**Two event thresholds, always both** (see [docs/fixes_2026-09_quote_and_event_floor.md](docs/fixes_2026-09_quote_and_event_floor.md) for the width-vs-events evidence behind the 300/1000 floors). The tracks entering the quote are the ones step 12 ran, i.e. at least `--nevt-standard` (300) events; the same board value is also recomputed on tracks with at least `--nevt-conservative` (1000), and both appear in the table with their pixel counts (and in the JSON under `boards.<role>.nevt_thresholds`). The headline number is the standard one - that is the set step 12 ran on. The conservative column exists because the converged mixture width is still ~5 % low at the 300-event standard threshold (~1 % at the 1000-event conservative one - see the width-vs-events study, notes/pipeline-pr-list.md 2026-09-07), so a board whose two numbers differ is a board resting on low-occupancy pixels, and that is now visible in the quote instead of having to be re-derived. Both need the step-11 `nevt_*.csv` next to the tables; without it the block records that reason and no threshold is applied.

**Pairing covariance (four boards, optional input).** The 3-board algebra assumes σ_ab² = σ_a² + σ_b². Measured on four-board coincidences that additivity is rejected (p = 8×10⁻¹¹ and 10⁻³⁰ on the two runs studied), while the one covariance model six pair widths *can* constrain - a single `g` shared by the two pairs of one **pairing** of the four boards, 5 parameters with 1 d.o.f. left to test it - fits them at χ²/dof 0.59 and 0.35, with g = +60 ps² at 1.5×10¹⁵ and +619 ps² on FBK at 3.5×10¹⁵. Under that model each board's three 3-board solves are v−g, v−g, v+g, so the combo-averaged quote is low by g/3 (+0.2 ps and +2 ps respectively). `quote_resolution.py` fits it when a four-board per-quadruple width table is available and stores the result **alongside** the standard solve (JSON key `pairing_covariance`: the best-supported pairing, g ± error, χ²/dof, the plain-additivity χ², the per-board σ from the 4-parameter LSQ and the g/3-corrected σ), never instead of it; when it cannot be computed the block says why. The table comes from `utils/fourboard_pairing.py`:
```bash
python utils/fourboard_pairing.py -t <EOS .../tracks_<run>> -n final_<run> -o final_<run>/fourboard_pairs_<run>.csv --workers 4
```
It rebuilds the four-board sample the pipeline never makes, by intersecting two 3-board combos on the raw hit codes of the two boards they share (the codes are recovered by inverting step 10's own ps conversion exactly), then fits the six pair widths per pixel quadruple with the pipeline's joint TWC over all four boards, its converged mixture and its acceptance gate, plus the two disjoint halves that calibrate the pair-width errors. `quote_resolution.py` picks up `fourboard_pairs_*.csv` next to the input tables automatically (or takes `--fourboard`), and requires `--pairing-min-events` (500) four-board events on `--pairing-min-quads` (30) quadruples - below that g is still measurable but loose (±35 %), so it is better quoted from a neighbouring run of the same configuration. This is a per-quadruple mixture sweep: run it on condor or a nice'd batch node, not interactively. Full derivation, controls and caveats: `notes/fourboard-lsq-test.md`.
...
| `fourboard_pairing.py` | Builds the four-board per-quadruple pair-width table that `quote_resolution.py`'s pairing-covariance fit consumes: intersects two 3-board combos on the shared boards' raw hit codes, then fits the six pair widths (and two disjoint halves, for the errors) per pixel quadruple with the pipeline's own TWC, mixture and acceptance gate. Heavy - run it on condor. |
