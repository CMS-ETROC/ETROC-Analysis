...
**Two event thresholds, always both.** The tracks entering the quote are the ones step 12 ran, i.e. at least `--nevt-standard` (100) events; the same board value is also recomputed on tracks with at least `--nevt-conservative` (300), and both appear in the table with their pixel counts (and in the JSON under `boards.<role>.nevt_thresholds`). The headline number is the standard one - that is the set step 12 ran on. The conservative column exists because the converged mixture width is still ~5 % low at ~300 events, so a board whose two numbers differ is a board resting on low-occupancy pixels, and that is now visible in the quote instead of having to be re-derived. Both need the step-11 `nevt_*.csv` next to the tables; without it the block records that reason and no threshold is applied.
